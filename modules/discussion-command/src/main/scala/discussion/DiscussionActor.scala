...
class DiscussionActor(val id: String) extends CommandActor {
  ...
  def updateState(event: DiscussionEvent): Unit = event match {
    case DiscussionStarted(startedId, userId, userName, blogId, title) =>
      state = Some(Discussion(startedId, blogId, title))
    case CommentAdded(_, commentId, userId, userName, content, index) =>
    ...
    case CommentReplied(_, replyId, userId, userName, parentId, content, path) =>
      state = state map { discussion =>
